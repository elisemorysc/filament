...
#include <gltfio/ReusableAnimator.h>
    ...
    for (size_t i = 0; i < mImpl->animations.size(); ++i) {
        for (size_t j =0; j < mImpl->animations[i].channels.size(); ++j) {
            mImpl->animations[i].channels[j].sourceData = mImpl->animations[i].samplers.data() 
                + (other.mImpl->animations[i].channels[j].sourceData
                    - other.mImpl->animations[i].samplers.data());
        }
    }
}

ReusableAnimator::~ReusableAnimator() {
    if (mImpl->assetToAnimate) removeAnimatedAsset();
    delete mImpl;
}

size_t ReusableAnimator::getAnimationCount() const {
    return mImpl->animations.size();
}

void ReusableAnimator::applyAnimation(size_t animationIndex, float time) const {
...
void ReusableAnimator::updateBoneMatrices() {
...
float ReusableAnimator::getAnimationDuration(size_t animationIndex) const {
    return mImpl->animations[animationIndex].duration;
}

const char* ReusableAnimator::getAnimationName(size_t animationIndex) const {
    return mImpl->animations[animationIndex].name.c_str();
}

void ReusableAnimator::removeAnimatedAsset() {
...
void ReusableAnimatorImpl::addChannels(const NamedEntityMap& entityMap, const cgltf_animation& srcAnim,
        Animation& dst) {
    ...
    for (cgltf_size j = 0, nchans = srcAnim.channels_count; j < nchans; ++j) {
        ...
        auto iter = entityMap.find(srcChannel.target_node->name);
        if (UTILS_UNLIKELY(iter == entityMap.end())) {
            if (GLTFIO_VERBOSE) {
...
void ReusableAnimatorImpl::applyAnimation(const Channel& channel, float t, size_t prevIndex,
        size_t nextIndex) {
    const Sampler* sampler = channel.sourceData;
    const TimeValues& times = sampler->times;
